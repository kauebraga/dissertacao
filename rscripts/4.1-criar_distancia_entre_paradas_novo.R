...
shapes <- st_read("../data/linhas/2018/linhas_2018-09.shp") %>%
...
shapes_stops_dists <- function(shapes_df, stops_linhas_df) {
  ...
  if (nrow(stops_linhas_df) == 2) {
    ...
    # ou..
    shapes_sp <- shapes_df %>% st_sf() %>% st_transform(29194) %>%
      as_Spatial()
    ...
    # Juntar
    stops_linhas_fim <- stops_linhas_ultimas %>%
      rbind(stops_linhas_ok %>% select(stop_id, linha_sentido, stop_sequence, lon, lat) %>%
                                         mutate(dist = dist))
